...
# ukb-b-7408 2018 Average total household income before tax  MRC-IEU 397751
# ukb-b-10011 2018 Townsend deprivation index at recruitment MRC-IEU 462464
...
TSMR_RF <- function(exposure_trait, outcome_trait){
	...
	if (outcome_trait == 'Years of schooling'){
		...
		outcome_dat = extract_outcome_data(snps = exp_dat$SNP, outcomes = 'ieu-b-35')
		outcome_dat$samplesize.outcome = 204402  # sample size information can be found in IEU OpenGWAS database
	} else if (outcome_trait == 'Household income'){
		outcome_dat = extract_outcome_data(snps = exp_dat$SNP, outcomes = 'ukb-b-7408')
		outcome_dat$samplesize.outcome = 397751  # sample size information can be found in IEU OpenGWAS database
	} else if (outcome_trait == 'Townsend deprivation index'){
		outcome_dat = extract_outcome_data(snps = exp_dat$SNP, outcomes = 'ukb-b-10011')
		outcome_dat$samplesize.outcome = 462464  # sample size information can be found in IEU OpenGWAS database
	}
...
# outcome_trait = 'Household income'
# outcome_trait = 'Townsend deprivation index'
outcome_list = c('Pack years of smoking', 'Alcohol intake frequency', 'Body mass index', 'Waist circumference', 'Time spent doing vigorous physical activity', 'C-Reactive protein level', "Household income", "Townsend deprivation index")
