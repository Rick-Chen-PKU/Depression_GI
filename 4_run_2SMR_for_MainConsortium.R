...
TSMR <- function(exposure_list, outcome_trait){ 
	for (exposure_trait in exposure_list){
		...
		if (file.exists(paste0('./', Project, '_MR_Result_main.xlsx'))){
            old_dat = read_excel(paste0('./', Project, '_MR_Result_main.xlsx'))
            new_dat = rbind(old_dat,"",out_df)
            write_xlsx(new_dat, paste0('./', Project, '_MR_Result_main.xlsx')) 
        } else {
            write_xlsx(out_df, paste0('./', Project, '_MR_Result_main.xlsx')) 
        } 
