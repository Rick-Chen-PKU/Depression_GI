...
# This script is a sensitivity analysis and  conduct main two-sample MR for FinnGen outcome after remove pleiotropy SNPs
# We searched the GWAS catalog (https://www.ebi.ac.uk/gwas/) to exclude pleiotropy SNPs (i.e. IVs for genome-wide significance for potentially confounding traits)
...
#---------------------------------------------------------------------#
#             Run two-sample MR  as sensitivity analysis              #----
#---------------------------------------------------------------------#
...
TSMR <- function(exposure_list, outcome_trait){ 
	for (exposure_trait in exposure_list){
		dat = data.frame(fread(paste0('./Project/', Project, '/', exposure_trait, '_to_', outcome_trait, '_FinnGen_instruments.csv')))  
		
		# remove pleiotropy SNPs
		if (exposure_trait=='CUD'){
			pleiotropy_snps =  c('rs11783093', 'rs1392816', 'rs7783012')
		} else if (exposure_trait=='MDD'){
			pleiotropy_snps =  c('rs10149470', 'rs11682175', 'rs12129573', 'rs1226412', 'rs12552', 'rs12958048', 'rs159963','rs1806153','rs5758265','rs61867293','rs7200826',
							'rs915057','rs9427672') 
		}
		dat =  dat[dat$SNP %in% setdiff(dat$SNP, pleiotropy_snps), ]; nrow(dat)
		...
		if (file.exists(paste0('./', Project, '_MR_Result_FinnGen_removePleioSNP.xlsx'))){
            old_dat = read_excel(paste0('./', Project, '_MR_Result_FinnGen_removePleioSNP.xlsx'))
            new_dat = rbind(old_dat,"",out_df)
            write_xlsx(new_dat, paste0('./', Project, '_MR_Result_FinnGen_removePleioSNP.xlsx')) 
        } else {
            write_xlsx(out_df, paste0('./', Project, '_MR_Result_FinnGen_removePleioSNP.xlsx')) 
        } 
