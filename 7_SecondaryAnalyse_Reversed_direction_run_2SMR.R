...
# This script conduct reversed two-sample MR to elucidate whether gastrointestinal disorders are potentially associated with depression or cannabis
...
TSMR <- function(exposure_list, outcome_trait){ 
	for (exposure_trait in exposure_list){
		...
		if (file.exists(paste0('./', Project, '_MR_Result_reversed.xlsx'))){
            old_dat = read_excel(paste0('./', Project, '_MR_Result_reversed.xlsx'))
            new_dat = rbind(old_dat,"",out_df)
            write_xlsx(new_dat, paste0('./', Project, '_MR_Result_reversed.xlsx')) 
        } else {
            write_xlsx(out_df, paste0('./', Project, '_MR_Result_reversed.xlsx')) 
        } 
...
exposure_list = c('GERD', 'IBS', 'PUD', 'NAFLD')
outcome_list = c('MDD', 'CUD')
